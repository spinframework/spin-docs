...
In Go, you register the handler as a callback in your program's `init` function.  Set `spinhttp.Handle` to your handler function.  Your handler takes a `ResponseWriter` and `*Request` pointer, and sends its response via the `ResponseWriter`.
